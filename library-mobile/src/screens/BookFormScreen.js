...
export default function BookFormScreen({ route, navigation }) {
  ...
  // Error flagovi za vizualno označavanje
  ...
  const validate = () => {
    ...
    if (isbn.trim() && !isbnRegex.test(isbn.trim())) {
      newErrors.isbn = true;
      valid = false;
    }

    if (publishedYear.trim() && (!/^\d{4}$/.test(publishedYear.trim()) || parseInt(publishedYear) < 0)) {
      newErrors.publishedYear = true;
      valid = false;
    }
